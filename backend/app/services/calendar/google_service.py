...
class GoogleCalendarService:
    def __init__(self, tenant: Any, redis_url: Optional[str] = None):
        ...
        try:
            ...
            self.redis = None
        self.refresh_token = getattr(tenant, "google_refresh_token", None)
        self.calendar_id = getattr(tenant, "google_calendar_id", None)
    ...
    def _rate_limit(self) -> bool:
        if not self.redis:
            return True
        try:
            key = f"tenant:{getattr(self.tenant, 'id', 'unknown')}:calendar_rl_google"
