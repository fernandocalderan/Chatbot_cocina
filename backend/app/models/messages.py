...
class Message(Base):
    ...
    __table_args__ = (sa.Index("ix_messages_session_created", "session_id", "created_at"),)

    id = sa.Column(sa.BigInteger, primary_key=True, autoincrement=True)
    tenant_id = sa.Column(pg.UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False)
    session_id = sa.Column(pg.UUID(as_uuid=True), sa.ForeignKey("sessions.id", ondelete="CASCADE"), nullable=False)
    ...
    attachments = sa.Column(pg.JSONB, nullable=False, server_default=sa.text("'[]'::jsonb"))
