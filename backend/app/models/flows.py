...
class Flow(Base):
    ...
    parent_flow_id = sa.Column(pg.UUID(as_uuid=True), nullable=True)
    subflow_key = sa.Column(sa.String(128), nullable=True)
    trigger_keywords = sa.Column(pg.JSONB, nullable=True)
