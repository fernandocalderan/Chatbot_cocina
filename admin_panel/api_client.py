...
def list_tenants(token: str):
    resp = requests.get(f"{API_BASE}/v1/admin/tenants", headers=_headers(token), timeout=10)
    return resp.json() if resp.ok else []


def create_tenant(token: str, payload: dict):
    resp = requests.post(f"{API_BASE}/v1/admin/tenants", json=payload, headers=_headers(token), timeout=10)
    return resp.json() if resp.ok else {"error": resp.text}


def update_tenant(token: str, tenant_id: str, payload: dict):
    resp = requests.patch(f"{API_BASE}/v1/admin/tenants/{tenant_id}", json=payload, headers=_headers(token), timeout=10)
    return resp.json() if resp.ok else {"error": resp.text}
...
def admin_overview(token: str):
    resp = requests.get(f"{API_BASE}/v1/admin/overview", headers=_headers(token), timeout=10)
    return resp.json() if resp.ok else {}


def impersonate(token: str, tenant_id: str):
    resp = requests.post(f"{API_BASE}/v1/admin/impersonate/{tenant_id}", headers=_headers(token), timeout=10)
    return resp.json() if resp.ok else {"error": resp.text}
